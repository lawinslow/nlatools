...
nla_chemistry = function(){
  ...
  mut2007 = nla2007 %>% select(SITE_ID, ANC, COND, DOC, NTL, PTL)
  mut2012 = nla2012 %>% mutate(ANC=ANC_RESULT, COND=COND_RESULT, DOC=DOC_RESULT, NTL=NTL_RESULT*1000, PTL=PTL_RESULT) %>%
    left_join(site2012) %>% select(SITE_ID, ANC, COND, DOC, NTL, PTL)

  return(merge(rbind(mut2007, mut2012), get_nla_sites()))
}
