...
big_table = plyr::join_all(list(tox_thin, lulc_thin, secchi_thin, site_thin, chem_thin, prof_surf, prof_deep), by=c('SITE_ID', 'VISIT_NO')) 
big_table = filter(big_table, VISIT_NO==1)

write.table(big_table, 'inst/extdata/bigtables/g19_2007_bigtable.csv', sep=',', row.names=FALSE)
